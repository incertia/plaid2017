...
with open("public.pem", "r") as f: key = RSA.importKey(f.read())
n = key.n
e = key.e
...
for i3 in xrange(0, m):
    for i2 in xrange(0, m):
        for i1 in xrange(0, 2 * (m - 1) - (i2 + i3) + t + 1):
            S.add(x ^ i1 * y ^ i2 * z ^ i3)
for i3 in xrange(0, m + 1):
    for i2 in xrange(0, m + 1):
        for i1 in xrange(0, 2 * m - (i2 + i3) + t + 1):
...
for col in xrange(nmonos):
    for row in xrange(col + 1, npolys):
...
assert p * q == n
assert Mod(e * d, 2 * g * a * b) == 1

n, e, d, p, q = map(int, (n, e, d, p, q))

key = RSA.construct((n, e, d, p, q))
cipher = PKCS1_OAEP.new(key)
with open("flag.enc", "r") as fi: ciphertext = fi.read()
